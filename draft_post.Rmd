...
example_words <- pronoun_ratio %>% 
    ...
    bind_rows(data_frame(word = c("cleans", "moans", "laughs", "twirls",
                                  "lingers", "bursts", "retreats", "answer",
...
pronouns <- c(sample(c("she", "She"), size = 36, replace = TRUE), 
              sample(c("he", "He"), size = 36, replace = TRUE))

example_text <- example_words %>%
    mutate(pronoun = pronouns,
           phrase = str_c(pronoun, word, sep = " ")) %>%
    pull(phrase) %>%
    map_df(function(x) all_lines %>%
               filter(str_detect(text, x)) %>%
               sample_n(1) %>%
               select(text)) %>%
    bind_cols(example_words) %>%
    select(word, text)
